...
public class Service : IService
{
    public List<penerbangan> getPenerbangan(string kota_asal, string kota_tujuan, DateTime tanggal_keberangkatan)
    {
        ...
        IEnumerable<penerbangan> daftar = from p in puyuh.penerbangan where ((p.kota_asal == kota_asal) && (p.kota_tujuan == kota_tujuan) && p.tanggal_keberangkatan == tanggal_keberangkatan) select p;
        if (daftar.Count() == 0)
        {
    ...
    public List<tiket> getPemesanan(int user_id)
    {
        ...
        tiket tmp = new tiket();
        puyuh_airEntities1 puyuh = new puyuh_airEntities1();
        IEnumerable<tiket> daftar = from t in puyuh.tiket where t.user_id == user_id select t;
        if (daftar.Count() == 0)
        {
            tiket a = new tiket();
            a.kode_bayar = "NULL";
