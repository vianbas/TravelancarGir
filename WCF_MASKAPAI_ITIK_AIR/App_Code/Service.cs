...
public class Service : IService
{
    ...
    public List<penerbangan> getPenerbangan(string kota_asal, string kota_tujuan, DateTime tanggal_keberangkatan)
    {
        ...
        itik_airEntities1 itik = new itik_airEntities1();
        IEnumerable<penerbangan> daftar = from p in itik.penerbangan where ((p.kota_asal == kota_asal) && (p.kota_tujuan==kota_tujuan) && p.tanggal_keberangkatan == tanggal_keberangkatan ) select p;
        if(daftar.Count()==0){
    ...
    {
        itik_airEntities1 itik = new itik_airEntities1();
        tiket tkt = itik.tiket.Find(id_tiket);
        tkt.status = "Lunas";
    ...
    public CompositeType getTiket(int id_tiket)
    {
        ...
        //daftar penumpang
        IEnumerable<penumpang> daftarPenumpang = from p in itik.penumpang where p.id_tiket == id_tiket select p;
        penumpang pn;
        ...
        //penerbangan
        penerbangan k = itik.penerbangan.Find(t.id_penerbangan);
        penerbangan pr = new penerbangan();
        pr.id = k.id;
        pr.harga = k.harga;
        pr.jam_keberangkatan = k.jam_keberangkatan;
    ...
    public List<tiket> getPemesanan(int user_id)
    {
        ...
        tiket tmp = new tiket();
        itik_airEntities1 itik = new itik_airEntities1();
        IEnumerable<tiket> daftar = from t in itik.tiket where t.user_id == user_id select t;
        if (daftar.Count() == 0)
        {
            tiket a = new tiket();
            a.kode_bayar = "NULL";
    ...
    public int booking(int id_penerbangan, List<penumpang> penumpangs, int user_id)
    {
        ...
        penerbangan p = itik.penerbangan.Find(id_penerbangan);
